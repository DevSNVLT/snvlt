...
{
    private $translator;
    public function __construct(TranslatorInterface $translator, private AdministrationService $administrationService)
    {
...
// Interfaces Admin
        if ($user->getCodeGroupe()->getId() == 1){
            ...
            $nb_reprises = $registry->getRepository(Reprise::class)->findBy(['exercice'=>$exercice]);
            $nb_autops = $registry->getRepository(AutorisationPs::class)->findBy(['exercice'=>$exercice]);
            $nb_autopv = $registry->getRepository(AutorisationPv::class)->findBy(['exercice'=>$exercice]);

            foreach ($nb_reprises as $reprise){
                $nb_autorisations = $nb_autorisations + 1;
            }
            foreach ($nb_autops as $autops){
                $nb_autorisations = $nb_autorisations + 1;
            }
            foreach ($nb_autopv as $autopv){
                $nb_autorisations = $nb_autorisations + 1;
            }
